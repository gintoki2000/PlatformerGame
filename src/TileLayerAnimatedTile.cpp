...
TileLayerAnimatedTile::TileLayerAnimatedTile(
    const std::vector<TextureRegion>& frames,
    const std::vector<unsigned int>&  intervals) :
    m_frames(frames),
    m_intervals(intervals), m_animationDuration(0)
{
...
void TileLayerAnimatedTile::UpdateBaseTime() { baseTicks = SDL_GetTicks(); }
